...
function Show-HyperVInventoryMenu {
    ...
    do {
        ...
        switch ($choice) {
            ...
            10 { Export-ToCSVFiles }            11 { New-HTMLReport }
...
function Invoke-CompleteInventoryCollection {
    ...
    try {
        ...
        $inventory.Storage = Get-HyperVStorageDetails
          Write-Host "[5/10] Collecting Network Adapter Information..." -ForegroundColor Cyan
        $inventory.Networks = Get-HyperVNetworkDetails
        
        Write-Host "[6/10] Collecting Performance Metrics..." -ForegroundColor Cyan
        $inventory.Performance = Get-HyperVPerformanceDetails
        
        Write-Host "[7/10] Collecting Security Configuration..." -ForegroundColor Cyan
        $inventory.Security = Get-HyperVSecurityDetails
        
        Write-Host "[8/10] Collecting Guest OS Information..." -ForegroundColor Cyan
        $inventory.GuestOS = Get-HyperVGuestOSDetails
        
        Write-Host "[9/10] Collecting Checkpoint Details..." -ForegroundColor Cyan
        $inventory.Checkpoints = Get-HyperVCheckpointDetails
        
        Write-Host "[10/10] Collecting Resource Pool & License Information..." -ForegroundColor Cyan
        $inventory.ResourcePools = Get-HyperVResourcePoolDetails
        $inventory.Licenses = Get-HyperVLicenseDetails
        
        # Save inventory data
        $inventoryFile = Join-Path $Global:ConfigPath "CompleteInventory_$timestamp.json"
        ...
        switch ($exportChoice.ToUpper()) {
            ...
            "H" { Generate-HTMLReport -InventoryData $inventory }
        }
...
function Export-ToRVToolsExcel {
    ...
    try {
        ...
        if (-not $InventoryData) {
            ...
            $inventoryFiles = Get-ChildItem -Path $Global:ConfigPath -Filter "CompleteInventory_*.json" | Sort-Object LastWriteTime -Descending
        ...
        # Export Guest OS data
        if ($InventoryData.GuestOS -and $InventoryData.GuestOS.Count -gt 0) {
            Write-Host "  Adding vGuestOS worksheet..." -ForegroundColor Cyan
            $InventoryData.GuestOS | Export-Excel -Path $excelFile -WorksheetName "vGuestOS" -AutoSize -TableStyle Medium2 -FreezeTopRow -BoldTopRow
        }
        
        # Export Checkpoint data  
        if ($InventoryData.Checkpoints -and $InventoryData.Checkpoints.Count -gt 0) {
            Write-Host "  Adding vSnapshots worksheet..." -ForegroundColor Cyan
            $InventoryData.Checkpoints | Export-Excel -Path $excelFile -WorksheetName "vSnapshots" -AutoSize -TableStyle Medium2 -FreezeTopRow -BoldTopRow
        }
        
        # Export Resource Pool data
        if ($InventoryData.ResourcePools -and $InventoryData.ResourcePools.Count -gt 0) {
            Write-Host "  Adding vResourcePools worksheet..." -ForegroundColor Cyan
            $InventoryData.ResourcePools | Export-Excel -Path $excelFile -WorksheetName "vResourcePools" -AutoSize -TableStyle Medium2 -FreezeTopRow -BoldTopRow
        }
        
        # Export License data
        if ($InventoryData.Licenses -and $InventoryData.Licenses.Count -gt 0) {
            Write-Host "  Adding vLicenses worksheet..." -ForegroundColor Cyan
            $InventoryData.Licenses | Export-Excel -Path $excelFile -WorksheetName "vLicenses" -AutoSize -TableStyle Medium2 -FreezeTopRow -BoldTopRow
        }
        ...
        Write-Host "Worksheets created: vHost, vInfo, vNetwork, vDisk, vNIC, vPerformance, vSecurity, vGuestOS, vSnapshots, vResourcePools, vLicenses, Summary" -ForegroundColor White
        ...
        Write-Log -Message "Error creating RVTools-style Excel file: $_" -Level "ERROR"
...
function Invoke-QuickEnvironmentScan {
    ...
    try {
        ...
        $vhdCount = (Get-ChildItem -Path "C:\*" -Include "*.vhd", "*.vhdx" -Recurse -ErrorAction SilentlyContinue | Measure-Object).Count
    ...
    try {
        ...
        if ($hostDetails -and $hostDetails.Count -gt 0) {
            ...
            foreach ($host in $hostDetails) {
                ...
                Write-Host "  Virtual Machines: $($host.TotalVMs)" -ForegroundColor White
            }
    ...
    try {
        ...
        if ($storageDetails -and $storageDetails.Count -gt 0) {
            ...
            $storageDetails | Sort-Object SizeGB -Descending | Select-Object -First 5 | ForEach-Object {
                Write-Host "  $($_.VHDPath): $([Math]::Round($_.SizeGB, 2)) GB" -ForegroundColor White
            }
        } else {
    ...
    try {
        ...
        if ($switchDetails -and $switchDetails.Count -gt 0) {
            ...
            foreach ($switch in $switchDetails) {
                ...
                Write-Host "    Connected VMs: $($switch.ConnectedVMCount)" -ForegroundColor Gray
            }
        }
        ...
        if ($networkDetails -and $networkDetails.Count -gt 0) {
            ...
            $connectedAdapters = ($networkDetails | Where-Object { $_.Connected -eq $true }).Count
            Write-Host "  Total Network Adapters: $($networkDetails.Count)" -ForegroundColor White
            Write-Host "  Connected Adapters: $connectedAdapters" -ForegroundColor White
        }
...
function Invoke-PerformanceMetricsCollection {
    ...
    try {
        ...
        if ($performanceData -and $performanceData.Count -gt 0) {
            ...
            # CPU Performance
            $avgCpuUsage = ($performanceData | Where-Object { $null -ne $_.CPUUsagePercent } | Measure-Object -Property CPUUsagePercent -Average).Average
            ...
            # Memory Performance
            $totalMemUsed = ($performanceData | Measure-Object -Property MemoryUsedGB -Sum).Sum
            Write-Host "`n💾 Memory Performance:" -ForegroundColor Yellow
            Write-Host "  Total Memory Used: $([Math]::Round($totalMemUsed, 2)) GB" -ForegroundColor White
            
            # Top performers
            Write-Host "`n🏆 Highest CPU Usage VMs:" -ForegroundColor Yellow
            $performanceData | Where-Object { $_.CPUUsagePercent -gt 0 } | Sort-Object CPUUsagePercent -Descending | Select-Object -First 3 | ForEach-Object {
                Write-Host "  $($_.Name): $($_.CPUUsagePercent)%" -ForegroundColor White
            }
        } else {
    ...
    try {
        ...
        if ($securityData -and $securityData.Count -gt 0) {
            ...
            $compliantVMs = ($securityData | Where-Object { 
                $_.SecureBootEnabled -eq $true -and $_.TmpEnabled -eq $true 
            }).Count
...
function Export-ToCSVFiles {
    ...
    try {
        ...
        if ($InventoryData.GuestOS) {
            $csvFile = Join-Path $exportFolder "GuestOS.csv"
            $InventoryData.GuestOS | Export-Csv -Path $csvFile -NoTypeInformation -Encoding UTF8
            Write-Host "  ✓ GuestOS.csv" -ForegroundColor Green
        }
        
        if ($InventoryData.Checkpoints) {
            $csvFile = Join-Path $exportFolder "Checkpoints.csv"
            $InventoryData.Checkpoints | Export-Csv -Path $csvFile -NoTypeInformation -Encoding UTF8
            Write-Host "  ✓ Checkpoints.csv" -ForegroundColor Green
        }
        
        if ($InventoryData.ResourcePools) {
            $csvFile = Join-Path $exportFolder "ResourcePools.csv"
            $InventoryData.ResourcePools | Export-Csv -Path $csvFile -NoTypeInformation -Encoding UTF8
            Write-Host "  ✓ ResourcePools.csv" -ForegroundColor Green
        }
        
        if ($InventoryData.Licenses) {
            $csvFile = Join-Path $exportFolder "Licenses.csv"
            $InventoryData.Licenses | Export-Csv -Path $csvFile -NoTypeInformation -Encoding UTF8
            Write-Host "  ✓ Licenses.csv" -ForegroundColor Green
        }
...
function New-HTMLReport {
    ...
    try {
        ...
        if ($InventoryData.Hosts) { $html += "<p>Hyper-V Hosts: $($InventoryData.Hosts.Count)</p>`n" }
        if ($InventoryData.VMs) { $html += "<p>Virtual Machines: $($InventoryData.VMs.Count)</p>`n" }
        if ($InventoryData.VirtualSwitches) { $html += "<p>Virtual Switches: $($InventoryData.VirtualSwitches.Count)</p>`n" }
        if ($InventoryData.Storage) { $html += "<p>VHD Files: $($InventoryData.Storage.Count)</p>`n" }
        $html += "</div>`n"
...
function New-ExecutiveSummary {
    ...
    try {
        ...
        $totalMemoryGB = if ($vms) { ($vms | Measure-Object -Property MemoryAssigned -Sum).Sum / 1GB } else { 0 }
...
$(if ($vmCount -gt 0) { "• Average Memory per VM: $([Math]::Round($totalMemoryGB / $vmCount, 2)) GB" })
...
function Invoke-EnvironmentHealthCheck {
    ...
    try {
        ...
        Write-Host "[1/5] Checking Hyper-V services..." -ForegroundColor Cyan
        ...
        # Check 2: VM Health
        Write-Host "[2/5] Checking VM health..." -ForegroundColor Cyan
        $vms = Get-VM -ErrorAction SilentlyContinue
        $vmCheck = @{
        ...
        if ($vms) {
            ...
            $vmIssues = $vms | Where-Object { $_.State -eq "Critical" -or $_.Status -ne "Operating normally" }
            if ($vmIssues) {
        ...
        Write-Host "[3/5] Checking storage health..." -ForegroundColor Cyan
        $storageCheck = @{
            Name = "Storage Health"
            Status = $true
            Details = @()
        ...
        Write-Host "[4/5] Checking network connectivity..." -ForegroundColor Cyan
        $networkCheck = @{
        ...
        # Check 5: Virtual Switch Health
        Write-Host "[5/5] Checking virtual switches..." -ForegroundColor Cyan
        $switchCheck = @{
...
# Enhanced data collection functions for RVTools compatibility

function Get-HyperVGuestOSDetails {
    <#
    .SYNOPSIS
        Collects guest OS information from VMs (requires Integration Services)
    #>
    try {
        $allGuestOS = @()
        ...
        foreach ($hostName in $hostNames) {
            try {
                Write-Host "    Collecting guest OS data from host: $hostName" -ForegroundColor Cyan
                
                $vms = Get-VM -ComputerName $hostName -ErrorAction SilentlyContinue | Where-Object { $_.State -eq "Running" }
                
                foreach ($vm in $vms) {
                    try {
                        # Get guest OS information through KVP (Key-Value Pair) exchange
                        $kvpItems = Get-VMKeyValuePairItem -VM $vm -ErrorAction SilentlyContinue
                        
                        $guestOSName = ($kvpItems | Where-Object { $_.Key -eq "OSName" }).Value
                        $guestOSVersion = ($kvpItems | Where-Object { $_.Key -eq "OSVersion" }).Value
                        $guestOSDistro = ($kvpItems | Where-Object { $_.Key -eq "OSDistributionName" }).Value
                        $guestFQDN = ($kvpItems | Where-Object { $_.Key -eq "FullyQualifiedDomainName" }).Value
                        ...
                        $guestInfo = [PSCustomObject]@{
                            HostName = $hostName
                            VMName = $vm.Name
                            GuestOSName = if ($guestOSName) { $guestOSName } else { "Unknown" }
                            GuestOSVersion = if ($guestOSVersion) { $guestOSVersion } else { "Unknown" }
                            GuestOSDistribution = if ($guestOSDistro) { $guestOSDistro } else { "Unknown" }
                            GuestFQDN = if ($guestFQDN) { $guestFQDN } else { "Unknown" }
                            IPAddresses = if ($ipAddresses.Count -gt 0) { $ipAddresses -join "; " } else { "Unknown" }
                            IntegrationServicesWorking = $vm.IntegrationServicesState -eq "UpToDate"
                        }
                        
                        $allGuestOS += $guestInfo
                    }
                    catch {
                        # Add entry even if we can't get guest info
                        $guestInfo = [PSCustomObject]@{
                            HostName = $hostName
                            VMName = $vm.Name
                            GuestOSName = "Unknown"
                            GuestOSVersion = "Unknown"
                            GuestOSDistribution = "Unknown"
                            GuestFQDN = "Unknown"
                            IPAddresses = "Unknown"
                            IntegrationServicesWorking = $false
                        }
                ...
                Write-Log -Message "Error collecting guest OS data from host $hostName`: $_" -Level "ERROR"
            }
        }
        ...
        Write-Log -Message "Error in Get-HyperVGuestOSDetails: $_" -Level "ERROR"
        return @()
    }
}

function Get-HyperVCheckpointDetails {
    <#
    .SYNOPSIS
        Collects detailed checkpoint/snapshot information
    #>
    try {
        $allCheckpoints = @()
        ...
        foreach ($hostName in $hostNames) {
            try {
                Write-Host "    Collecting checkpoint data from host: $hostName" -ForegroundColor Cyan
                ...
                foreach ($vm in $vms) {
                    try {
                        ...
                        foreach ($checkpoint in $checkpoints) {
                            # Calculate checkpoint file sizes
                            $checkpointSizeGB = 0
                            try {
                                if (Test-Path $checkpoint.Path) {
                                    $checkpointFiles = Get-ChildItem -Path $checkpoint.Path -Recurse -Force -ErrorAction SilentlyContinue
                                    $checkpointSizeGB = [Math]::Round(($checkpointFiles | Measure-Object -Property Length -Sum).Sum / 1GB, 2)
                                }
                            }
                            catch {
                                $checkpointSizeGB = 0
                            ...
                            $checkpointInfo = [PSCustomObject]@{
                                ...
                                SizeGB = $checkpointSizeGB
                                Id = $checkpoint.Id
                                VMId = $checkpoint.VMId
                            }
                        ...
                        Write-Log -Message "Error collecting checkpoint data for VM $($vm.Name): $_" -Level "WARNING"
                    }
                }
            }
            catch {
                Write-Log -Message "Error collecting checkpoint data from host $hostName`: $_" -Level "ERROR"
            }
        }
        ...
        Write-Log -Message "Error in Get-HyperVCheckpointDetails: $_" -Level "ERROR"
        return @()
    }
}

function Get-HyperVResourcePoolDetails {
    <#
    .SYNOPSIS
        Collects resource pool information
    #>
    try {
        $allResourcePools = @()
        ...
        foreach ($hostName in $hostNames) {
            try {
                Write-Host "    Collecting resource pool data from host: $hostName" -ForegroundColor Cyan
                
                # Get memory resource pools
                $memoryPools = Get-VMResourcePool -ComputerName $hostName -ResourcePoolType Memory -ErrorAction SilentlyContinue
                ...
                foreach ($pool in $memoryPools) {
                    $poolInfo = [PSCustomObject]@{
                        ...
                        ParentName = $pool.ParentName
                        AllocationUnits = $pool.AllocationUnits
                        Capacity = $pool.Capacity
                        Reserved = $pool.Reserved
                        PoolId = $pool.PoolId
                    }
                    
                    $allResourcePools += $poolInfo
                }
                
                # Get processor resource pools
                $processorPools = Get-VMResourcePool -ComputerName $hostName -ResourcePoolType Processor -ErrorAction SilentlyContinue
                ...
                Write-Log -Message "Error collecting resource pool data from host $hostName`: $_" -Level "ERROR"
            }
        }
        
        return $allResourcePools
    }
    catch {
        Write-Log -Message "Error in Get-HyperVResourcePoolDetails: $_" -Level "ERROR"
        return @()
    }
}

function Get-HyperVLicenseDetails {
    <#
    .SYNOPSIS
        Collects Windows activation and license information
    #>
    try {
        $allLicenses = @()
        ...
        foreach ($hostName in $hostNames) {
            try {
                Write-Host "    Collecting license data from host: $hostName" -ForegroundColor Cyan
                
                # Get host license information
                $licenseInfo = Get-WmiObject -Class SoftwareLicensingProduct -ComputerName $hostName | 
                    Where-Object { $_.PartialProductKey -and $_.LicenseStatus -eq 1 } | 
                    Select-Object -First 1
                
                $hostLicense = [PSCustomObject]@{
                    HostName = $hostName
                    Type = "Host"
                    VMName = "N/A"
                    ProductName = if ($licenseInfo) { $licenseInfo.Name } else { "Unknown" }
                    LicenseStatus = if ($licenseInfo) { 
                    ...
                    PartialProductKey = if ($licenseInfo) { $licenseInfo.PartialProductKey } else { "Unknown" }
                    LicenseFamily = if ($licenseInfo) { $licenseInfo.LicenseFamily } else { "Unknown" }
                }
                
                $allLicenses += $hostLicense
                
                # Get guest VM license information (for running VMs with Integration Services)
                $vms = Get-VM -ComputerName $hostName -ErrorAction SilentlyContinue | Where-Object { $_.State -eq "Running" }
                
                foreach ($vm in $vms) {
                    try {
                        # Try to get guest license info through KVP
                        $kvpItems = Get-VMKeyValuePairItem -VM $vm -ErrorAction SilentlyContinue
                        $guestLicenseStatus = ($kvpItems | Where-Object { $_.Key -eq "OSLicenseStatus" }).Value
                        $guestProductName = ($kvpItems | Where-Object { $_.Key -eq "OSName" }).Value
                        
                        $vmLicense = [PSCustomObject]@{
                            HostName = $hostName
                            Type = "Guest"
                            VMName = $vm.Name
                            ProductName = if ($guestProductName) { $guestProductName } else { "Unknown" }
                            LicenseStatus = if ($guestLicenseStatus) { $guestLicenseStatus } else { "Unknown" }
                            PartialProductKey = "N/A"
                            LicenseFamily = "N/A"
                        }
                        
                        $allLicenses += $vmLicense
                    }
                    catch {
                        # Add placeholder entry for VMs we can't query
                        $vmLicense = [PSCustomObject]@{
                            HostName = $hostName
                            Type = "Guest"
                            VMName = $vm.Name
                ...
                Write-Log -Message "Error collecting license data from host $hostName`: $_" -Level "ERROR"
            }
        }
        ...
        Write-Log -Message "Error in Get-HyperVLicenseDetails: $_" -Level "ERROR"
        return @()
    }
}
